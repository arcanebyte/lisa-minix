...
#if (MACHINE == LISA)
  /* If build put a RAM disk size in FS's data space, the boot block loaded
   * the root file system image right after INIT, which is where MM puts the
   * RAM disk, and nothing needs to be copied.  Otherwise the root file
   * system is /dev/hd0, the file system area of the ProFile.
   */
  count = data_org[INFO + 3];
  if (count != 0) {
	super_dev = DEV_RAM;
	root_device = DEV_RAM;
	goto got_root_dev;
  }
  boot_parameters.bp_rootdev = DEV_HD0;
...
#if (MACHINE == LISA)
  printf("RAM disk: %u blocks, loaded by the boot block.\n\n", count);
  return(super_dev);
#endif
