...
/* Handle sys_fork().  m_ptr->PROC1 has forked.  The child is m_ptr->PROC2. */
  ...
  *rpc = *rpp;
  ...
  rpc->child_stime = 0;
#if (MACHINE == LISA)
  /* No shadowing: copy the parent's whole image (text, data, gap, stack) to
   * the memory MM allocated, and give the child the same virtual addresses
   * at the new physical ones.  The MMU maps them (lisa/lisammu.c).
   */
  {
	phys_clicks child_base, nc;
	int seg;

	child_base = (phys_clicks) m_ptr->m1_p1;
	nc = rpp->p_map[S].mem_phys - rpp->p_map[T].mem_phys +
						rpp->p_map[S].mem_len;
	copyclicks(rpp->p_map[T].mem_phys, child_base, nc);
	for (seg = T; seg <= S; seg++)
		rpc->p_map[seg].mem_phys = child_base +
		    (rpp->p_map[seg].mem_phys - rpp->p_map[T].mem_phys);
	rpc->p_nflips = 0;
  }
#else
#if (CHIP == M68000)
...
/* Calculate the physical memory address for a given virtual address. */
  ...
  if((vir_addr>>CLICK_SHIFT) >= rp->p_map[seg].mem_vir + rp->p_map[seg].mem_len)
	return( (phys_bytes) 0 );
#if (CHIP == INTEL) || (MACHINE == LISA)
  seg_base = (phys_bytes) rp->p_map[seg].mem_phys;
  seg_base = seg_base << CLICK_SHIFT;	/* segment origin in bytes */
#endif
  pa = (phys_bytes) vir_addr;
#if (CHIP != M68000) || (MACHINE == LISA)
  /* On the Lisa virtual and physical addresses differ (lisa/lisammu.c). */
...
/* Return the base and size of the next chunk of memory of a given type. */
  ...
  if (beenhere)
	...
	/* Memory from logical 0 to the end of RAM reported by the boot ROM,
	 * less the video page at the top (lisamain.c). */
	m_ptr->COUNT = (phys_clicks) (lisa_user_mem_end() >> CLICK_SHIFT);
	beenhere++;
  }
#endif /* (MACHINE == LISA) */
