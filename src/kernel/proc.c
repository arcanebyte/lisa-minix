 ...
 *				cp_mess					    *
 *==========================================================================*/
PUBLIC void cp_mess(src, src_p, src_m, dst_p, dst_m)
...
#if (MACHINE == LISA)
  /* Message pointers are virtual addresses in the sender and receiver;
   * translate them to physical ones (lisa/lisammu.c).  For tasks the two
   * are the same.
   */
  src_m = (message *)((char *)src_m +
	(((phys_bytes)src_p->p_map[D].mem_phys << CLICK_SHIFT) -
	 ((phys_bytes)src_p->p_map[D].mem_vir << CLICK_SHIFT)));
  dst_m = (message *)((char *)dst_m +
	(((phys_bytes)dst_p->p_map[D].mem_phys << CLICK_SHIFT) -
	 ((phys_bytes)dst_p->p_map[D].mem_vir << CLICK_SHIFT)));
#endif
