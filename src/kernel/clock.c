...
PRIVATE void init_clock()
{
/* The Lisa's clock tick is the vertical retrace interrupt, turned on in
 * initlisa() (lisa/lisamain.c).  Ask the COPS for the date and time; the
 * answer arrives as keyboard interrupt bytes, and lisa/lisakbd.c calls
 * clock_set_boot_time() with it.
 */
  lisa_read_rtc();
}
 ...
 *				clock_set_boot_time			     *
 *===========================================================================*/
PUBLIC void clock_set_boot_time(t)
time_t t;			/* current time, seconds since 1970 */
{
/* Set the time of day from the Lisa's real-time clock.  Called at interrupt
 * level; ticks not yet counted into realtime are a fraction of a second.
 */
  boot_time = t - realtime/HZ;
}
#endif
