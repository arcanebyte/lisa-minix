...
#if (MACHINE == LISA)
  /* No shadowing: the kernel copies the whole image, text included, to the
   * child's memory, and the MMU maps it at the parent's virtual addresses.
   */
  prog_clicks += rmp->mp_seg[T].mem_len;
#endif
#if (CHIP == INTEL)
