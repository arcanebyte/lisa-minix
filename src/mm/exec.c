 ...
 */
  ...
  if (relocate(fd, mbuf) < 0)
	;	/* error */
#endif
  /* On the Lisa, programs run at virtual address 0, where they are linked,
   * so the relocation information is not needed.
   */
...
#if (MACHINE == LISA)
  /* Every program at virtual address 0; the MMU maps it (lisammu.c). */
  rmp->mp_seg[T].mem_vir = 0;
  rmp->mp_seg[D].mem_vir = text_clicks;
  rmp->mp_seg[S].mem_vir = text_clicks + data_clicks + gap_clicks;
#else
#if (CHIP == M68000)
