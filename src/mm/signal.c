 ...
 */
  ...
  for (i = 0, j = 1; i < _NSIG - 1; i++, j++) {
	...
	if ( (sig_map >> i) & 1) {
#if (MACHINE == LISA)
		/* A memory fault trapped by the MMU (the kernel sends SIGSEGV
		 * for a bus error, SIGBUS for an address error) always kills:
		 * the 68000 cannot restart the faulting instruction.
		 */
		if (j == SIGSEGV || j == SIGBUS) {
			rmp->mp_catch &= ~(1 << (j - 1));
			rmp->mp_ignore &= ~(1 << (j - 1));
		}
#endif
