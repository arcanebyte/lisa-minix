 ...
 */
...
#if (MACHINE == LISA)
  /* The Lisa MMU maps 512-byte pages (src/kernel/lisa/lisammu.c): blocks
   * start at even clicks and are an even number of clicks long, and
   * free_mem() rounds lengths the same way.
   */
  clicks = (clicks + 1) & ~1;
#endif
  hp = hole_head;
  while (hp != NIL_HOLE) {
#if (MACHINE == LISA)
	if (hp->h_base & 1) {		/* cannot happen; never use such a hole */
		prev_ptr = hp;
		hp = hp->h_next;
		continue;
	}
...
#if (MACHINE == LISA)
  clicks = (clicks + 1) & ~1;		/* as alloc_mem() allocated it */
#endif
