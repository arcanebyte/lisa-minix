...
/* MACHINE must be set to one of the machine types list below. The build
 * can override it (-DMACHINE=LISA). */
#ifndef MACHINE
...
#define LISA              63	/* Apple Lisa 2 (68000) */
...
#define VT100		   100
#define NR_DRIVES	   1	/* the ProFile on the parallel port (lisapro.c) */
#endif
...
#if (MACHINE == ATARI) | (MACHINE == AMIGA) | (MACHINE == MACINTOSH) | (MACHINE == LISA)
#define CHIP          M68000
#endif
