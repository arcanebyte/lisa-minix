...
/* The ACK compiler this tree was built with was not ANSI, so _ANSI was 0.
 * gcc always defines __STDC__ as 1; building with -D_MINIX_KR keeps the
 * K&R declarations and argument promotion the code was written against.
 */
#ifdef __STDC__			/* __STDC__ defined for (near) ANSI compilers*/
#if __STDC__ == 1 && !defined(_MINIX_KR) /* == 1 for conformant compilers */
